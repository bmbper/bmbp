...
pub async fn role_index_view(_req: &mut Request, res: &mut Response) {
    let ctx = page_context("rbac/role/index");
    let te = BMBP_TEMPLATE.render("page.html", &ctx).unwrap();
    res.render(Text::Html(&te))
}

#[handler]
pub async fn role_config_view(req: &mut Request, res: &mut Response) {
    let mut ctx = page_context("rbac/role/config");
    let page_params = req
...
#[handler]
pub async fn role_base_view(_req: &mut Request, res: &mut Response) {
    let ctx = page_context("rbac/role/base");
    let te = BMBP_TEMPLATE.render("page.html", &ctx).unwrap();
    res.render(Text::Html(&te))
}
