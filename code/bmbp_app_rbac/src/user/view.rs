...
pub async fn user_index_view(_req: &mut Request, res: &mut Response) {
    let ctx = page_context("rbac/user/index");
    let te = BMBP_TEMPLATE.render("page.html", &ctx).unwrap();
    res.render(Text::Html(&te))
}

#[handler]
pub async fn user_config_view(req: &mut Request, res: &mut Response) {
    let mut ctx = page_context("rbac/user/config");
    let page_params = req
...
#[handler]
pub async fn user_base_view(_req: &mut Request, res: &mut Response) {
    let ctx = page_context("rbac/user/base");
    let te = BMBP_TEMPLATE.render("page.html", &ctx).unwrap();
    res.render(Text::Html(&te))
}
