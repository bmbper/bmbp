use bmbp_app_common::{BmbpResp, BmbpTreeModel, BmbpPageParam, PageVo};
...
impl BmbpSettingDictService {
    ...
    pub async fn find_dict_page(params: &BmbpPageParam<DictQueryParams>) -> BmbpResp<PageVo<BmbpSettingDictOrmTreeModel>> {
        let query_sql = BmbpCurdScript::query::<BmbpSettingDictOrmTreeModel>();
        tracing::info!("query_sql: {}", query_sql.build());
