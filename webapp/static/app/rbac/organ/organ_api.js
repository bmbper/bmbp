...
const onQueryGridData = (queryData) => {
  ...
  OrganAppIns.setPagination({ ...OrganAppIns.pagination, total: data.length });
  OrganAppIns.setGridData(data);
...
const onSearchFormQueryBtnClick = () => {
  var queryData = OrganAppIns.formRef.current.getFieldsValue();
  onQueryGridData(queryData);
}

const onSearchFormRestBtnClick = () => {
  OrganAppIns.formRef.current.resetFields();
}

/// 列表分页大小变化
const onGridPageChange = (page) => {
  OrganAppIns.setPagination({ ...OrganAppIns.pagination, pageSize: page.pageSize });
}
